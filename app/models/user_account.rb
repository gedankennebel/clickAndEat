class UserAccount < ActiveRecord::Base
  ...
  validates :name, size: {minumum: 2, message: "A Name shoud contain at least 2 letters!"}
  ...
  validates :password, format: {with:((?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,20}), 
   message: "A password should match following pattern: 
   6 to 20 characters string with at least one digit, one upper case letter and one lower case letter!"}
  ...
  validates_associated :roles, :restaurants 
end
